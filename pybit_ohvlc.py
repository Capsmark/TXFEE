...
import csv
import pandas as pd
import pybit as bybit
from datetime import datetime


def date_to_milliseconds(iso_string):
    try:
        if iso_string.lower() == "now":
        ...
        raise ValueError(
            "Invalid ISO date format. Please provide a valid ISO-formatted date string or 'now' for the current time.")
...
def get_historical_klines(symbol, interval, start_str, end_str=None):
    ...
    while True:
        ...
        temp_dict = bybit.kline(
            symbol=symbol, interval=timeframe, _from=start_ts, limit=limit)
...
def get_historical_klines_pd(symbol, interval, start_str, end_str=None):
    ...
    df['Date'] = [datetime.fromtimestamp(i).strftime(
        '%Y-%m-%d %H:%M:%S.%d')[:-3] for i in df['TimeStamp']]

    return df


get_historical_klines_pd('BTCUSD', "D", "2015-01-01T00:00:00Z")
